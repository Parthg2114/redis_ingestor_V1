"""
Universal MQTT Client Module for Heterogeneous Sensor Data

Supports real sensors, industrial simulators, and mixed IoT environments
with automatic payload format detection and normalization.
"""
...
class UniversalMQTTClient:
    """Universal MQTT Client with heterogeneous sensor payload support."""
    ...
    def __init__(self, on_message_callback: Optional[Callable] = None):
        ...
        # Payload format detection patterns
    ...
    # ---------------------------------------------------------------------
    # Client setup/connection helpers
    # ---------------------------------------------------------------------
    ...
    def _apply_normalization(self, data: Union[Dict, List], topic: str, mqtt_ts: float, fmt: str) -> Dict[str, Any]:
        ...
        if fmt == 'industrial_simulator':
            ...
            base.update({'device_id': self._clean(str(dev)), 'timestamp': self._parse_ts(ts) or mqtt_ts, 'sensor_data': sensor})
        elif fmt == 'nested_data':
            dev = data.get('deviceId') or data.get('device_id') or data.get('deviceID')
            ts = data.get('timestamp')
            base.update({'device_id': self._clean(str(dev)), 'timestamp': self._parse_ts(ts) or mqtt_ts, 'sensor_data': data.get('data', {})})
        elif fmt == 'topic_based':
            dev = self._device_from_topic(topic)
            ts = data.pop('timestamp', None)
            base.update({'device_id': dev, 'timestamp': self._parse_ts(ts) or mqtt_ts, 'sensor_data': data})
            ...
            base.update({'device_id': dev, 'timestamp': self._parse_ts(ts) or mqtt_ts, 'sensor_data': sensor})
